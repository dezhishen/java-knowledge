...
### 修饰参数
Java允许在参数列表中以声明的方式将参数指明为final，这意味这你**无法在方法中更改参数引用所指向的对象**。这个特性主要用来向匿名内部类传递数据。
...
```java
    ...
    private static class FinalDemo {
        private int a = 0;         // 普通域
        ...
        public static void reader() {
            ...
            if (demo != null) {
                ...
                int b = demo.b;
                if (a != 1) {
                    System.out.println("普通变量a == 0"); // 非1则说明有机会读到0
                } else {
                    System.out.println("普通变量a == 1"); // 非1则说明有机会读到0
                }
                if (b != 2) {
                    System.out.println("final变量b = " + b); // 理论上不会出现
                }
            }
        }
    }
    ...
    public static void main(String[] args) throws InterruptedException {
        // 增加次数，提高可见概率
        for (int i = 0; i < 100; i++) {
            Thread writer = new Thread(FinalDemo::writer);
            Thread reader = new Thread(FinalDemo::reader);
            writer.start();
            reader.start();
            writer.join();
            reader.join();
...
sequenceDiagram
    ...
    线程A->>线程A: 构造函数开始执行
    线程A->>线程A: 写final域b=2
    线程A->>线程A: StoreStore屏障
    线程A->>线程A: 构造函数执行完毕
    线程A->>线程A: 把构造对象引用赋值给finalDemo

    线程B->>线程B: 读对象引用finalDemo
    线程B->>线程B: 读普通域a，此时读到空值，即分配内存时的0值
    线程B->>线程B: 读final域b
    线程A->>线程A: 写普通变量域a=1
```
...
### final域为引用类型
#### 对`final修饰的对象`的成员域写操作
针对引用数据类型，final域写针对编译器和处理器重排序增加了这样的约束：在构造函数内对一个final修饰的对象的成员域的写入，与随后在构造函数之外把这个被构造的对象的引用赋给一个引用变量，这两个操作是不能被重排序的。注意这里的是“增加”也就说前面对final基本数据类型的重排序规则在这里还是适用。这句话是比较拗口的，下面结合实例来看。
```java
...
sequenceDiagram
    ...
    participant 线程C
    线程A->>线程A: 构造函数开始执行
    线程A->>线程A: 1.写final类型引用
    线程A->>线程A: 2.写final引用对象的成员域
    线程C->>线程C: 5.读对象引用
    线程C->>线程C: LoadLoad屏障
    线程A->>线程A: StoreStore屏障
    线程A->>线程A: 构造函数执行完毕
    线程C->>线程C: 5.读final引用对象的成员域
    线程A->>线程A: 3.把构造对象的引用赋值给引用变量
    线程B->>线程B: 4.写final引用对象的成员域
```

由于对final域的写禁止重排序到构造方法外，因此1和3不能被重排序。由于一个final域的引用对象的成员域写入不能与随后将这个被构造出来的对象赋给引用变量重排序，因此2和3不能重排序。

> 即：线程A中，一定是先完成了内部的 `1`，`2`，对象才会对外可见。

#### 对`final修饰的对象`的成员域读操作
JMM可以确保线程C至少能看到写线程A对final引用的对象的成员域的写入，即能看下arrays[0] = 1，而写线程B对数组元素的写入可能看到可能看不到。JMM不保证线程B的写入对线程C可见，线程B和线程C之间存在数据竞争，此时的结果是不可预知的。如果可见的，可使用锁或者volatile。

### 关于final重排序的总结
> 对象被发布前，final引用字段及其成员的初始化赋值一定都已完成。这就是JMM对final引用安全发布的保证。
...
#### 引用数据类型：
- **额外增加**约束：禁止在构造函数对一个final修饰的对象的成员域的写入与随后将这个被构造的对象的引用赋值给引用变量 重排序
...
> “溢出”/“逸出”是什么意思？指一个对象还没构造完成、还没安全初始化，它的引用就已经“逃逸”到构造函数之外，被外部访问到了。在并发环境下，其他线程可能会拿到这个对象，并访问这个对象的状态，这可能会得到“未初始化”或“部分初始化”状态的数据（即初始零值等）。
...
- referenceDemo = this; 这一行是在构造函数尚未完成时，就把当前对象引用发布到外部（假如外部能通过某种方式访问referenceDemo）。
- 如果别的线程这时通过referenceDemo拿到此对象，就可能在构造函数还没完全执行完之前，访问到a仍然是默认值0的状态。
- 尽管a被final修饰，但JVM对final域的“安全发布”规则是在构造函数“正常返回”后才生效！如果对象还未构造完成就被暴露出去，JVM的final域安全保障机制就会失效。
...
```java
myList = new ArrayList(); // 报错
myList = someOtherList; // 报错
```
...
- final修饰的基本类型且赋初值，编译器会当做常量看待，直接在编译期做计算和优化，允许隐式类型转换。
- 普通变量不是常量，编译器不能确定运行时值，不会做隐式降级类型，需要你显式强转。