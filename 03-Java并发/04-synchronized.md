...
1. **实例方法**：锁的是当前实例对象 `this`，属于对象锁  
2. **静态方法**：锁的是当前类的 `Class` 对象，属于类锁  
3. **代码块**：锁的是括号里指定的对象，可以是对象锁也可以是类锁
...
- `synchronized` 依赖对象的**监视器（monitor）**实现互斥  
- 进入同步块相当于执行 `monitorenter`，退出同步块相当于执行 `monitorexit`  
- 如果监视器被占用，其他线程会竞争失败并进入阻塞等待，直到持有者释放
...
- **释放锁**之前，会把该线程在临界区内对共享变量的更新刷新出去  
- **获取锁**之后，后续读共享变量能看到之前释放锁线程的更新
...
3. **无法实现公平策略**  
   - `synchronized` 不提供公平性选择  
   - `ReentrantLock` 可构造公平锁（吞吐通常会降低）

4. **条件队列不够灵活**  
   - `synchronized` 只能配合 `wait` 与 `notify` 或 `notifyAll`，且只有一个等待集合的表达方式  
   - `Lock` 搭配 `Condition` 可以创建多个条件队列，更精细地唤醒

5. **锁释放时机不够灵活**  
   - `synchronized` 只能在作用域结束自动释放  
   - `Lock` 可以在任意位置 `unlock()`（但必须保证最终释放）
...
1. 尽量缩小同步范围，只包住共享数据的关键操作  
2. 不要在同步块内执行耗时操作（网络、磁盘、长时间计算）  
3. 避免锁对象频繁变化，锁对象应当稳定  
4. 避免锁顺序不一致导致死锁（多把锁时固定获取顺序）  
5. 注意类锁影响范围更大，容易放大竞争
...
1. **减小锁粒度**：大锁拆小锁，减少无关竞争  
2. **减小锁持有时间**：把耗时操作移出同步块  
3. **读多写少用读写锁**：用 `ReentrantReadWriteLock` 提升读并发  
4. **无锁与原子类**：例如 `AtomicInteger` 或 `LongAdder` 等  
5. **并发容器**：用并发集合替代手写锁
...
- 可以在更灵活的位置 `lock()` 与 `unlock()`  
- 可以配合 `tryLock` 做回退策略  
- 可以配合 `Condition` 做多条件等待与精确唤醒
