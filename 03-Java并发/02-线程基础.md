...
```mermaid
graph LR
    创建一个线程对象 --"new Thread()"--> New
    New --"Thread.start()"--> Runnable
    Runnable --"Lock"--> Blocked
    Blocked --"UnLock" --> Runnable
    Runnable --"Object.wait(time)"--> Timed_Waiting
    Timed_Waiting --"时间到了，或者被notify" -->Runnable
    Runnable --"Object.wait()"--> Waiting
    Waiting --"Object.notify()"-->Runnable
    Runnable --> Terminated
```
