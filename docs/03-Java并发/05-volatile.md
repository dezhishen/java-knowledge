...
导致的问题：多线程环境下就可能将一个未初始化的对象引用暴露出来，从而导致不可预料的结果。
...
⬆️这段代码的问题在于，线程A执行时，拿到的`stop`已经在线程的缓冲区中（且拿到的是`false`），而后续主线程处理的`stop`对象，不会再写入线程A的`stop`，会导致无限的自旋下去。
...
因为long和double两种数据类型的操作可分为**高32位和低32位两部分**，因此普通的long或double类型读/写可能不是原子的。因此，鼓励大家将共享的long和double变量设置为volatile类型，这样能保证任何情况下对long和double的单次读/写操作都具有原子性。
...
> 缓存行是中央处理器缓存里进行数据存放和交换的最小单位，可以理解为缓存把**内存**按固定大小的“块”来管理，这个“块”就是缓存行

### volatile 有序性实现
#### volatile 的 happens-before 关系
happens-before 规则中有一条是 volatile 变量规则：对一个 volatile 域的写，happens-before 于**任意后续**对这个 volatile 域的**读**。
...
class VolatileExample {
    ...
    public void reader() {
        ...
        int i = a;          // 4 线程B读共享变量
        // 此处3一定比4先
        // 注意，此处的先决条件是 flag 确实为 true ，可以外部自旋来保障
        ……
        }
    }
}
...
根据上面的代码可知，如果线程B观测到 `flag` 为`true` 则`a`必定已经完成赋值。

## volatile 的应用场景
使用 volatile 必须具备的条件
- 对变量的写操作不依赖于当前值。
    - 如果依赖当前值，因为`volatitle`只保证单一读写的原子性，如果依赖当前值，虽然读取的时候，是正确的值，但再后续的过程中，可能发生另外的线程将"当前值"进行了修改。
- 该变量没有包含在具有其他变量的不变式中。
    ```java
    class Range {
        private int start = 0;
        ...
        public boolean isValid() {
            // 线程B可能看到 end 已更新，但 start 还没更新
            return start <= end;
        }
    }
    ```

    > 此处 start 是非`volatile`变量符修饰。
...
> 注意：此处是通过让“读”的过程不存在锁的任何资源消耗，提升的性能。
...
⬆️在上面的代码，可以通过`final`实现，即：饿汉式。此处不存在绝对的优劣，只看场景。